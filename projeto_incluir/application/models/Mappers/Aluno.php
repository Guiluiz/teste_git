    ...
    public function getAlunosNotas($turma) {
        try {
            ...
            $select = $this->db_aluno->select()
                    ...
                    ->joinLeft('atividade', 'turma_atividades.id_atividades_turma = atividade.id_atividade', array())
                    ->joinLeft('turma', 'turma.id_turma = turma_alunos.id_turma', array('nome_turma', 'id_disciplina'))
                    ->where('turma.id_turma = ?', (int) $turma)
                    ->where('aluno.status = ?', Application_Model_Aluno::$status_ativo)
                    ->where('turma_atividades.id_turma = turma_alunos.id_turma')
                    ->order('aluno.nome_aluno ASC');
            ...
            if ($this->verificaLancamentos($quantidade_alunos_turma, $calendario_atual, $turmas_datas_lancamentos, $ids_atividades_turma, $notas_lancadas)) {
                if ($calendario_atual instanceof Application_Model_DatasAtividade) {
                    ...
                    if ($periodo_atual instanceof Application_Model_Periodo && $this->verificaLancamentos($quantidade_alunos_turma, $calendario_atual, $turmas_datas_lancamentos, $ids_atividades_turma, $notas_lancadas)) {
                        ...
                        foreach ($alunos as $aluno) {
                            ...
                            foreach ($turmas_aluno as $id_turma => $turma) {
                                 ...
                                 $select = $this->db_turma->select()
                                  ->setIntegrityCheck(false)
                                  ->from('turma', array('nome_turma'));
                                 $select->where('turma.id_turma = ?',(int)$id_turma);
                                 $nomeTurma = $this->db_turma->fetchAll($select);
                                 
                                 if (!empty($nomeTurma)) {
                                     $array_nomeTurma = array();
                                     foreach ($nomeTurma as $turmaNome )
                                    //  $nomeAp = $nomeAps['nome_turma'];
                                        $nomeAp = $turmaNome['nome_turma'];
                                 }
                                        ...
                                        if ($aluno->getNotaAcumulada($id_turma, false, true) >= $periodo_atual->getMinPtsAprovacao() && $aluno->getPorcentagemFaltas($id_turma, $calendario_atual->getQuantidadeAulas()) * 100 >= $periodo_atual->getFrequenciaLiberacao()){
                                            ...
                                            echo $aluno->getNomeAluno().';'. $aluno->getNotaAcumulada($id_turma, false, true).' ;'.
                                                $periodo_atual->getMinPtsAprovacao(). ' ;'.$aluno->getPorcentagemFaltas($id_turma, $calendario_atual->getQuantidadeAulas(), TRUE * 100).
                                                 ';'.$periodo_atual->getFrequenciaLiberacao().' ;'. $nomeAp.'; APROVADO <br />';
                                 ...
                                 //           );
                                        ...
                                        //$array_nomeTurma[$i] = $aluno->getNomeAluno().';'.$aluno->getNotaAcumulada($id_turma,false,true);
                                        echo $aluno->getNomeAluno().';'. $aluno->getNotaAcumulada($id_turma, false, true).' ;'.
                                           $periodo_atual->getMinPtsAprovacao(). ' ;'.$aluno->getPorcentagemFaltas($id_turma, $calendario_atual->getQuantidadeAulas(), TRUE * 100).
                                          ';'.$periodo_atual->getFrequenciaLiberacao().' ;'. $nomeAp.'; REPROVADO <br />';
                        ...
                        // $resultado = $excel->getRelatorioFechamentoNotas($array_nomeTurma, $formato_saida);
                        //print_r($array_nomeTurma);
                        return true;
                    }
                }
            }
